...
function ProjetosPage() {
  ...
  const emAprovacao = projetos.filter((projeto) =>
    tarefas.some((tarefa) => {
          ...
          <section className="order-1">
            ...
            <div className="flex gap-2 overflow-x-auto px-0.5 py-1.5">
              {clientesOrdenados.map((nome) => {
                ...
                const destino = ps.find((p) => !isProjetoConcluidoOuPausado(p)) ?? ps[0];
                return (
                  <button
                    key={nome}
                    draggable
                    ...
                    onClick={() => {
                      ...
                      if (workspaceId)
                        navigate({ to: "/projetos/$id", params: { id: workspaceId } });
                    }}
                    style={{ "--cliente": cor } as React.CSSProperties}
                    className={cn(
                      "group relative min-w-[240px] rounded-xl border bg-surface-1/40 p-4 text-left transition-[transform,border-color,background-color,box-shadow,opacity] duration-200 hover:z-10 hover:scale-[1.015] hover:border-[var(--cliente)] hover:bg-surface-1/65 hover:shadow-[0_12px_30px_-18px_var(--cliente)]",
                      cliente === nome
                  ...
                  >
                    <span
                      className="absolute right-2.5 top-2.5 cursor-grab text-muted-foreground/45 transition group-hover:text-muted-foreground active:cursor-grabbing"
                      aria-label="Arrastar para reordenar"
                    >
                      <GripVertical size={16} />
                    </span>
                    <div className="flex items-center gap-3">
                      <span className="grid size-9 place-items-center rounded-lg bg-[color-mix(in_srgb,var(--cliente)_16%,transparent)] text-xs font-bold text-[var(--cliente)]">
                        {iniciais(nome)}
                      </span>
