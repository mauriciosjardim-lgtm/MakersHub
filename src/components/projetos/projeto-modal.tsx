...
export function ProjetoModal({
  ...
  const salvar = async () => {
    ...
    const payload = {
      ...
      fase: projeto ? projeto.fase : "briefing",
      equipe,
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-xl grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden p-0">
        ...
        <DialogFooter className="flex-row items-center justify-between gap-2 border-t border-border/60 bg-background px-4 py-3 sm:justify-between sm:px-6 sm:py-4">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={salvar} disabled={salvando || periodoInvalido}>
      ...
      <Dialog open={confirmandoRemocao} onOpenChange={(v) => !v && setConfirmandoRemocao(false)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-display">Remover projeto?</DialogTitle>
            <DialogDescription>
              Esta ação remove o projeto e todos os seus entregáveis, marcos e tarefas.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button variant="destructive" onClick={remover} disabled={removendo}>
              {removendo ? "Removendo..." : "Remover projeto"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
