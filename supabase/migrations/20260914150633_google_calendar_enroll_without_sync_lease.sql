-- Enrolling a freshly-created local event only writes its pending mapping. It does
-- not call Google and is safe alongside an in-flight reconciliation. Keeping it
-- behind the reconciliation lease caused a one-shot browser request to be lost
-- whenever the minute sync happened at the same time.
